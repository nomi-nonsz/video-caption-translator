...
program
  ...
  .option('-m, --model <model>', "specify model for translating (example: -m anthropic/claude-opus-4-8). if no model is specified, the first model will be used.")
  .option('-s, --size <number>', "number of cues in a chunk", optionParser.size, "15")
  .option("--think", "enhance higher thinking and model reasoning, translation process might take longer")
  .option("--temperature <float>", "control the response randomness", optionParser.temperature)
